# Hi, I'm Ram Prasad Somaraju 👋

### Applied AI Engineer | Building AI-Driven Onboarding & Support Tools

I'm an **Applied AI Engineer** based in **Limerick, Ireland**, passionate about building intelligent systems that transform how products onboard and support users.

---

## 🚀 What I Do

- Building **AI-powered onboarding and customer support tools** at **Wigmore IT Group**
- Designing **LLM-based agents** using LangChain, Mistral, and other cutting-edge frameworks
- Integrating cloud infrastructure with **GCP and Azure** for scalable SaaS products
- Creating data-driven product experiences with **Next.js, React, and SQL**
- Implementing **Gainsight PX** for analytics and user engagement tracking

---

## 🛠️ Tech Stack & Tools

| Category | Technologies |
|----------|-------------|
| **Languages** | JavaScript, TypeScript, Python, SQL |
| **Frontend** | Next.js, React |
| **AI/LLM** | LangChain, CrewAI, LLMs, Mistral, RAG, Agentic AI |
| **AI Coding Tools** | Claude Code, Codex, Lovable, Emergent, Gemini ADK |
| **Cloud** | Google Cloud Platform (GCP), Azure, Supabase |
| **Tools** | Gainsight PX and CS, Slack, GitHub Actions, Staircase AI |

---

## 💼 Projects & Work

### 🐾 1-Place Pet Platform (In Progress)
Building a comprehensive pet care platform with my co-founder **Laxman Prasad Somaraju** ([@laxmanprasadsomaraju](https://github.com/laxmanprasadsomaraju)). A one-stop platform for all things pets!

### 🌐 PortfolioPal - CV to Portfolio Generator
I built a tool that converts your CV into a beautiful portfolio. Try it out and create your own portfolio in minutes!
🔗 [portfolio-pal.ie](https://preview--portfolio-pal-ie.lovable.app/)

---

## 🌐 Portfolio & Links

- 🎨 **My Portfolio**: [portfolio-pal.ie/ram-prasad-somaraju](https://preview--portfolio-pal-ie.lovable.app/p/ram-prasad-somaraju-emjx65)
- 💼 **LinkedIn**: [linkedin.com/in/ramprasad26](https://www.linkedin.com/in/ramprasad26/)
- 🐙 **GitHub**: [github.com/dataanalystram](https://github.com/dataanalystram)
- 👨‍💻 **Co-Founder**: [github.com/laxmanprasadsomaraju](https://github.com/laxmanprasadsomaraju)

---

## 📍 About Me

- 🏠 Based in **Limerick, Ireland** 🇮🇪 with strong ties to **India** 🇮🇳
- 🎯 Focused on **Applied AI, Product Onboarding, and Cloud Computing**
- 🤝 Actively contributing to AI hackathons and tech conferences (Gainsight Pulse, Mistral Global AI Hackathon)
- 📈 Passionate about **product-led growth** and user engagement analytics
- 🎬 Love watching thrillers, comedy movies, and analyzing cricket matches

---

## 📊 GitHub Stats

![Streak Stats](https://streak-stats.demolab.com/?user=dataanalystram&theme=radical&hide_border=true)

![Activity Graph](https://ghchart.rshah.org/dataanalystram)

---

<p align="center">
  <em>"Building intelligent products, one commit at a time."</em>
</p>
  <em>"Building intelligent products, one commit at a time."</em>
</p>
